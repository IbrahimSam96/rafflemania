            ...
            <>
                <span className={`flex ml-2`}  >
                    ...
                    <span className={`flex mt-2 overflow-y-hidden 
                        ...
                        <p className={`text-[#6FCF97] inline font-[Meta] text-xl sm:text-2xl ml-2 `}>
                            {parseInt(mania.balance._hex) == 0 ? '0.00' : ethers.utils.formatEther(mania.balance._hex)} {chain == 'Polygon' ? 'MATIC' : chain == 'Ethereum' ? 'ETH' : chain == 'Binance' ? 'BNB' : chain == "Avalanche" ? 'AVAX' : ''}</p>
